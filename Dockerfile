...
RUN sed -i 's/archive.ubuntu.com/mirrors.tuna.tsinghua.edu.cn/g' /etc/apt/sources.list && \
    ...
    apt update && \
    apt upgrade -y && \
    apt install -y vim tree locate iproute2 zip unzip genisoimage grub-pc-bin grub-efi-amd64-bin dosfstools initramfs-tools \
    rsync bsdmainutils gdisk psmisc ncdu strace symlinks && \
